...
router.get('/2016/:kierrosId', function(req, res) {
  ...
  db.query('SELECT m.* FROM sarjataulukkodata m LEFT JOIN sarjataulukkodata b ON m.team_id = b.team_id AND m.kierros_id < b.kierros_id AND b.kierros_id < ? AND b.season_id = 1 WHERE b.kierros_id IS NULL AND m.kierros_id < ? m.season_id = 1 ORDER BY m.team_id', [kierros, kierros], function(err, rows, fields) {
      console.log('Connection result error '+err);
      if (err) throw err;
